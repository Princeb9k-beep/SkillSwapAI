...
function CommunityDetail({ id, onBack }) {
  ...
  return (
    <section>
      ...
      {data.posts.length === 0 ? (
        <EmptyState icon="chat" title="No posts yet" hint="Be the first to post — introduce yourself or ask a question." />
      ) : (
        <div className="posts">
          {data.posts.map((p) => (
...
export default function Communities() {
  ...
  return (
    <section>
      ...
      {list.length === 0 ? (
        <EmptyState icon="sprout" title="No communities yet" hint="Create the first one above." />
      ) : (
        <div className="grid">
          {list.map((c) => (
